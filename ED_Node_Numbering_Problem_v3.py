...
# --------------------
# node numbering "simple" + with ANY constraints variant                ==> results in correct periods/frequencies (T1 = 0.407 s)

# node numbering "problematicV1" + constraints "zeroLengthSection       ==> results in correct periods/frequencies (T1 = 0.407 s)
# node numbering "problematicV1" + constraints "zeroLength"             ==> results in correct periods/frequencies (T1 = 0.407 s)
# node numbering "problematicV1" + constraints "twoNodeLink"            ==> results in correct periods/frequencies (T1 = 0.407 s)

# node numbering "problematicV2" + constraints "zeroLengthSection       ==> results in correct periods/frequencies (T1 = 0.407 s)
# node numbering "problematicV2" + constraints "zeroLength"             ==> results in correct periods/frequencies (T1 = 0.407 s)
# node numbering "problematicV2" + constraints "twoNodeLink"            ==> results in correct periods/frequencies (T1 = 0.407 s)

# node numbering "problematicv3" + constraints "zeroLengthSection       ==> results in correct periods/frequencies (T1 = 0.407 s)
# node numbering "problematicV3" + constraints "zeroLength"             ==> results in correct periods/frequencies (T1 = 0.407 s)
# node numbering "problematicV3" + constraints "twoNodeLink"            ==> results in correct periods/frequencies (T1 = 0.407 s)


# INCORRECT combinations
# ----------------------
# node numbering "problematicV1" + constraints "equalDOF"               ==> results in INCORRECT periods/frequencies (T1 = 0.167 s)
# node numbering "problematicV1" + constraints "rigidLink"              ==> results in INCORRECT periods/frequencies (T1 = 0.167 s)

# node numbering "problematicV2" + constraints "equalDOF"               ==> results in INCORRECT periods/frequencies (T1 = 0.326 s)
# node numbering "problematicV2" + constraints "rigidLink"              ==> results in INCORRECT periods/frequencies (T1 = 0.326 s)

# node numbering "problematicV3" + constraints "equalDOF"               ==> results in INCORRECT periods/frequencies (T1 = 0.298 s)
# node numbering "problematicV3" + constraints "rigidLink"              ==> results in INCORRECT periods/frequencies (T1 = 0.298 s)
...
# Constraints type
# ----------------
constraints = "equalDOF" # "equalDOF", "rigidLink", "zeroLength", "zeroLengthSection", "twoNodeLink"
...
if nodeNumbering == "simple":
    ...
    response as if the model doesn't take it into account the masses in Node4 and in Node6 !
    
    Original node numbering generated by the preprocessor
    """
    node1 = 11 # 11
    node2 = 111 # 111
    
    node3 = 1110101 # 1110101
    node4 = 11101 # 11101
    
    node5 = 1110201 # 1110201
    node6 = 11102 # 11102
    
elif nodeNumbering == "problematicV2":
    """
    This combination of node numbering gives equal frequencies and dynamic
    response as if the model only takes into account the mass in Node4 but not in Node6 !
    """
    node1 = 1 # 11
    node2 = 11 # 111
    ...
    """
    This combination of node numbering gives equal frequencies and dynamic
    response as if the model only takes into account the mass in Node6 but not in Node4 !
    """
    node1 = 1
    node2 = 11
...
# Nodes
# -----
ops.node(node1, 0.00, 0.00, 0.00,   '-mass', *[0.802*tonne, 0.802*tonne, 0.802*tonne, 0, 0, 0]) # Node 1
ops.node(node2, 0.00, 0.00, 3.12*m, '-mass', *[1.420*tonne, 1.420*tonne, 1.420*tonne, 0, 0, 0]) # Node 2

ops.node(node4, 0.25*m, 0.00,   3.12*m, '-mass', *[3.906*tonne, 3.906*tonne, 3.906*tonne, 0, 0, 0]) # Node 4 
ops.node(node6, 0.00,   0.20*m, 3.12*m, '-mass', *[3.071*tonne, 3.071*tonne, 3.071*tonne, 0, 0, 0]) # Node 6

# ops.node(node4, 0.25, 0.00, 3.12, '-mass', *[0, 0, 0, 0, 0, 0]) # Node 4 
# ops.node(node6, 0.00, 0.20, 3.12, '-mass', *[0, 0, 0, 0, 0, 0]) # Node 6

ops.node(node3, 0.00, 0.00, 3.12*m, '-mass', *[0, 0, 0, 0, 0, 0]) # Node 3
ops.node(node5, 0.00, 0.00, 3.12*m, '-mass', *[0, 0, 0, 0, 0, 0]) # Node 5

# Restraints
# ----------
ops.fix(node1, *[1, 1, 1, 1, 1, 1]) # Node 1
...
# ---------------------------------------------------
ops.geomTransf('PDelta', 1, *[0.0, 1.0, 0.0], '-jntOffset', *[0,0,0], *[0,0,0])

# ops.section('Elastic', 1, *[E, E*A, Iz, Iy, G, J])
# ops.beamIntegration('Legendre', 10, 1, 5)
# ops.element('forceBeamColumn', 1, *[node1, node2], 1, 10, '-mass', 0.0) # Nodes 1 & 2

ops.element("elasticBeamColumn", 1, *[node1, node2], A, E, G, J, Iy, Iz, 1)

# Constraints
# -----------
if constraints == "equalDOF":
    ops.equalDOF(node2, node3, *[1,2,3,4,5,6]) # Nodes 2 & 3
    ops.equalDOF(node2, node5, *[1,2,3,4,5,6]) # Nodes 2 & 5
    ops.equalDOF(node3, node4, *[1,2,3,4,5,6]) # Nodes 3 & 4
    ops.equalDOF(node5, node6, *[1,2,3,4,5,6]) # Nodes 5 & 6
    ...
    ops.element('zeroLength', 121, *[node2, node3], '-mat', *[999,999,999,999,999,999], '-dir', *[1,2,3,4,5,6])
    ops.element('zeroLength', 122, *[node2, node5], '-mat', *[999,999,999,999,999,999], '-dir', *[1,2,3,4,5,6])
    ops.element('zeroLength', 123, *[node3, node4], '-mat', *[999,999,999,999,999,999], '-dir', *[1,2,3,4,5,6])
    ops.element('zeroLength', 124, *[node5, node6], '-mat', *[999,999,999,999,999,999], '-dir', *[1,2,3,4,5,6])

elif constraints == "zeroLengthSection":
    ops.section('Aggregator',600, 999,'T',999,'P',999,'Mz',999,'Vy',999,'My',999,'Vz')
    
    ops.element('zeroLengthSection', 121, *[node2, node3], 600)
    ops.element('zeroLengthSection', 122, *[node2, node5], 600)
    ops.element('zeroLengthSection', 123, *[node3, node4], 600)
    ops.element('zeroLengthSection', 124, *[node5, node6], 600)
    
elif constraints == "twoNodeLink":
    ops.element('twoNodeLink', 121, *[node2, node3], '-mat', *[999,999,999,999,999,999], '-dir', *[1,2,3,4,5,6])
    ops.element('twoNodeLink', 122, *[node2, node5], '-mat', *[999,999,999,999,999,999], '-dir', *[1,2,3,4,5,6])
    ops.element('twoNodeLink', 123, *[node3, node4], '-mat', *[999,999,999,999,999,999], '-dir', *[1,2,3,4,5,6])
    ops.element('twoNodeLink', 124, *[node5, node6], '-mat', *[999,999,999,999,999,999], '-dir', *[1,2,3,4,5,6])
...
for i in range(0, num_modes):
    title(
        f"Mode #{i+1} ==> T{i+1} = {period[i]:1.3f} sec, f{i+1} = {freq[i]:1.3f} Hz, ω{i+1} = {omega[i]:1.3f} rad/s")
...
tFinal = GMdata["npts"] * GMdata["dt"]  # integer of the number of steps needed
