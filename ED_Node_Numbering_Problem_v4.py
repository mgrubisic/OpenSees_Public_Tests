...
if nodeNumbering == "simple":
    ...
    + Pairs of nodes (node3+node4 & node5+node6) constrained with the equalDOF/rigidLink command one after the other in a sequence.
    """
    node1 = 1
    node2 = 2
    ...
    + Pairs of nodes (node3+node4 & node5+node6) constrained with the equalDOF/rigidLink command one after the other in a sequence.
    + A consistent pattern was observed: if the tag of the constrained/secondary node is smaller than
      that of its corresponding retained/primary node, the mass associated with the constrained/secondary node
...
# Nodes
# -----
ops.node(node1, 0.0, 0.0, 0.0, '-mass', *[0, 0, 0, 0, 0, 0]) # Node 1
ops.node(node2, 0.0, 0.0, 3*m, '-mass', *[2*tonne, 2*tonne, 0, 0, 0, 0]) # Node 2

ops.node(node3, 0.0, 0.0, 3*m, '-mass', *[2*tonne, 2*tonne, 0, 0, 0, 0]) # Node 3
ops.node(node5, 0.0, 0.0, 3*m, '-mass', *[2*tonne, 2*tonne, 0, 0, 0, 0]) # Node 5

ops.node(node4, 0.0, 0.0, 3*m, '-mass', *[4*tonne, 4*tonne, 0, 0, 0, 0]) # Node 4 
ops.node(node6, 0.0, 0.0, 3*m, '-mass', *[8*tonne, 8*tonne, 0, 0, 0, 0]) # Node 6 

# Duplicate nodes 3, 5 & 4, 6 without masses, for testing!
# --------------------------------------------------------
# ops.node(node4, 0.0, 0.0, 3*m, '-mass', *[0, 0, 0, 0, 0, 0]) # Node 4 
# ops.node(node6, 0.0, 0.0, 3*m, '-mass', *[0, 0, 0, 0, 0, 0]) # Node 6 
